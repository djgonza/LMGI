					...
					<tbody id="CTRL2">
						<xsl:for-each select="Vuelos">
							...
							<tr>
								...
								<td>
									<xsl:value-of select="fx:FormatFromXSL('Hora_Salida', 'Long Time', '', '', 7)"/>
								</td>
								<td>
									<xsl:value-of select="Distancia"/>
								</td>
								<td>
									<xsl:value-of select="Tipo_Avion"/>
								</td>
							</tr>
						</xsl:for-each>
					</tbody>
